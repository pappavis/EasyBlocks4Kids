...
Blockly.Arduino['text_append'] = function(block) {
    ...
    try {
        ...
        varName + ' = String(' + varName + ') + String(' + value + ');\n';
        code1 = varName;
    } catch (error) {
        alert("text_append: " + error);
    }
    return code1;
};
    ...
    switch (where) {
        ...
        case 'RANDOM':
            var functionName = Blockly.Arduino.provideFunction_(
                'textRandomLetter', ['function ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
                    '(text) {',
...
Blockly.Arduino['text_getSubstring'] = function(block) {
    ...
    if (where1 == 'FIRST' && where2 == 'LAST') {
        ...
        var functionName = Blockly.Arduino.provideFunction_(
            'subsequence' + wherePascalCase[where1] + wherePascalCase[where2], ['function ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
                '(sequence' +
...
Blockly.Arduino['text_changeCase'] = function(block) {
    ...
    if (operator) {
        ...
        var functionName = Blockly.Arduino.provideFunction_(
            'textToTitleCase', ['function ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
                '(str) {',
...
Blockly.Arduino['text_count'] = function(block) {
    ...
    var sub = Blockly.Arduino.valueToCode(block, 'SUB',
        Blockly.Arduino.ORDER_NONE) || '\'\'';
    var functionName = Blockly.Arduino.provideFunction_(
        'textCount', ['function ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
            '(haystack, needle) {',
            '  if (needle.length === 0) {',
...
Blockly.Arduino['text_replace'] = function(block) {
    ...
    var functionName = Blockly.Arduino.provideFunction_(
        'textReplace', ['function ' + Blockly.Arduino.FUNCTION_NAME_PLACEHOLDER_ +
